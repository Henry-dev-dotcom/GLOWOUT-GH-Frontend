  ...
  return (
    <>
      <section className="relative min-h-[760px] overflow-hidden pt-20">
        ...
        <div className="container-lux relative flex min-h-[720px] items-center">
          <div className="max-w-3xl">
            ...
            <div className="mt-10 grid max-w-xl grid-cols-3 gap-3">
              <StatCard label="Products" value={products.length} hint="Managed in admin" />
              <StatCard label="Delivery" value="GH" hint="Configurable fees" />
              <StatCard label="Checkout" value="MoMo" hint="Ready for gateway" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-surface-1/45 py-16"><div className="container-lux"><div className="mb-8"><p className="section-eyebrow">Featured Today</p><h2 className="heading-md mt-2">Products your customers can buy now</h2></div><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">{featured.map((p) => <ProductCard key={p.id} product={p} navigate={navigate} compact />)}</div></div></section>
      <section className="py-16"><div className="container-lux grid gap-10 lg:grid-cols-2 lg:items-center"><div className="h-[460px] rounded-3xl product-image" style={{ backgroundImage: `url(${imageBank.about})` }} /><div><p className="section-eyebrow">The GLOWOUT GH Promise</p><h2 className="heading-lg mt-3">Luxury feeling, practical shopping.</h2><p className="mt-5 text-lg leading-9 text-[#C8BAD0]">This React build keeps the original store logic while separating the frontend into reusable parts. Product management, categories, orders, coupons, returns, customers, settings and finance are now easier to maintain.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><Card className="p-5"><h3 className="font-bold text-gold">Authentic Product Flow</h3><p className="mt-2 text-sm leading-6 text-[#8A7A98]">Images, details, stock and availability update across the store.</p></Card><Card className="p-5"><h3 className="font-bold text-gold">Admin-Ready Structure</h3><p className="mt-2 text-sm leading-6 text-[#8A7A98]">The frontend is prepared for backend and payment integration later.</p></Card></div></div></div></section>
    </>
  );
}
