      ...
      <section className="pb-16">
        <div className="container-lux grid gap-8 lg:grid-cols-[320px_1fr]">
          ...
          <div className="grid gap-6">
            ...
            {activeTab === 'security' && (
              <Card className="p-6"><h3 className="font-display text-2xl font-bold">Security</h3><p className="mt-2 text-sm leading-6 text-[#8A7A98]">Change your account password. Use at least 8 characters with a mix of letters and numbers.</p><form onSubmit={updatePassword} className="mt-6 grid gap-4"><Field label="Current Password" type="password" value={securityForm.currentPassword} onChange={(e) => setSecurityForm({ ...securityForm, currentPassword: e.target.value })} /><div className="grid gap-4 md:grid-cols-2"><Field label="New Password" type="password" value={securityForm.nextPassword} onChange={(e) => setSecurityForm({ ...securityForm, nextPassword: e.target.value })} /><Field label="Confirm New Password" type="password" value={securityForm.confirmPassword} onChange={(e) => setSecurityForm({ ...securityForm, confirmPassword: e.target.value })} /></div>{securityMessage && <p className="rounded-xl border border-gold/20 bg-gold/10 p-3 text-sm text-gold">{securityMessage}</p>}<Button className="w-fit" type="submit">Update Password</Button></form></Card>
            )}
          </div>
        </div>
      </section>
