...
export function AnalyticsPage({ view, navigate }) {
  ...
  return (
    <AdminShell view={view} navigate={navigate} title="Sales Analytics">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2"><Card className="p-6"><SectionTitle title="Customer Segments" /><BarList items={customerSegments} /></Card><Card className="p-6"><SectionTitle title="Actionable Insights" /><div className="space-y-3 text-sm text-[#C8BAD0]"><p className="rounded-2xl bg-surface-2 p-4">Low stock products: <b className="text-gold">{products.filter((p) => safeNumber(p.stock) <= safeNumber(settings.lowStockThreshold)).length}</b>. Review Product Manager before new campaigns.</p><p className="rounded-2xl bg-surface-2 p-4">Average order value is <b className="text-gold">{money(aov, settings.currency)}</b>. Use coupons or bundles to lift basket size.</p><p className="rounded-2xl bg-surface-2 p-4">Refund exposure is <b className="text-gold">{money(refunds, settings.currency)}</b>. Review Returns Manager for repeated reasons.</p></div></Card></div>
    </AdminShell>
  );
}
